...
AGENT_INSTRUCTIONS = (
    ...
    "用户明确描述刚发生的饮食、饮水、活动、作息、环境或就医/服药事实时，提出 life_record；"
    "medical 只记录已发生的就医或服药事实，绝不补全药名、剂量、疗程或诊断。"
...
class ConversationAgentProvider(ChatProvider):
    ...
    def chat(self, model: str, messages: Sequence[Mapping]) -> str:
        ...
        # The existing Aliyun v3 proxy deliberately has no native tool API.
        # Keep one ordinary chat request and the reviewed JSON draft contract;
        # do not probe unsupported endpoints, retry, or require server upgrades.
        ...
        system["content"] += (
            "\n本通道使用单次结构化兼容模式，不调用外部工具。"
            "只输出 answer 和 proposals 的 JSON 对象；所有建议需用户确认后保存。"
        )
        ...
        self._check_cancelled()
        parsed = AiAssistantService._parse_response(result, allowed_types=MEMBER_PROPOSAL_TYPES)
        toolkit = SnapshotTools({}, input_source=self.input_source)
        self.trace = ["structured_single_response"]
        for proposal in parsed["proposals"]:
            toolkit.execute(
                "propose_action", json.dumps({"proposal": proposal}, ensure_ascii=False)
            )
        self.trace.extend(toolkit.trace)
        parsed["proposals"] = toolkit.proposals
