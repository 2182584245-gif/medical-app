...
class SnapshotTools:
    ...
    def execute(self, name: str, raw_arguments: str) -> dict:
        ...
        if name == "propose_action" and set(args) == {"proposal"}:
            ...
            if proposal["type"] == "life_record" and proposal["category"] != "medical":
                # Origin is assigned by the trusted input adapter, never by the model.
                # Medical details intentionally have a stricter existing allowlist;
                # its provenance remains the service's ai_confirmed source field.
