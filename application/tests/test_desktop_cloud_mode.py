...
from ollama_chat_app.services.network_status import NetworkStatusMonitor
...
def isolated_connection_settings(tmp_path, monkeypatch):
    ...
    # Connection routing is tested here; the separate health-probe worker is
    # exercised with its own injected transport in test_network_status.py.
    monkeypatch.setattr(NetworkStatusMonitor, "start", lambda self: None)

    def forbidden_network(*_args, **_kwargs):
        raise AssertionError("Cloud-mode tests must never open a real network connection")
...
def _user(name):
    ...
    return {
        "id": 1 if name == "synthetic-first" else 2,
        "username": name,
...
def test_cloud_window_builds_remote_services_and_virtual_commerce_without_local_business_data(
    ...
    window = build_desktop_window(
        "cloud",
        local_database=ForbiddenDatabase(),
    ...
    for service in (
        window.auth_service,
        window.chat_service,
        window.health_service,
        window.service_management_service,
        window.preferences_service,
        window.file_management_service,
        window.ai_assistant_service,
        window.commerce_service,
    ):
    ...
    assert requests == []
    assert (
        window.health_workspace.service_page.commerce_panel.commerce_service
        is window.commerce_service
    )
...
def test_cloud_user_change_and_restart_do_not_reuse_tokens_or_keys(
    ...
    factory, clients, requests = mock_cloud
    controller = DesktopWindowController(
        synthetic_database, client_factory=factory, endpoint_store=local_endpoint_store
    )
    ...
    restarted = DesktopWindowController(
        synthetic_database, client_factory=factory, endpoint_store=local_endpoint_store
    )
...
def test_fresh_install_starts_default_aliyun_without_network_or_config_write(
    ...
    controller = DesktopWindowController(
        synthetic_database, endpoint_store=store, client_factory=factory
    )
    window = controller.start()
    qtbot.addWidget(window)
...
def test_saved_local_mode_starts_local_without_any_cloud_client(
    ...
    synthetic_database,
    mock_cloud,
    local_endpoint_store,
):
    factory, clients, requests = mock_cloud
    before = local_endpoint_store.path.read_bytes()
    controller = DesktopWindowController(
        synthetic_database, endpoint_store=local_endpoint_store, client_factory=factory
    )
    window = controller.start()
    qtbot.addWidget(window)
...
def test_custom_aliyun_route_survives_restart_and_reaches_the_same_dialog_store(
    qtbot,
    synthetic_database,
    mock_cloud,
    tmp_path,
    ...
    saved = EndpointSettings("cloud", "aliyun", "https://first.example.com/api")
    store.save(saved)
    ...
    for _ in range(2):
        ...
        window = controller.start()
        qtbot.addWidget(window)
        assert clients[-1].base_url == saved.aliyun_url
        assert window.login_page.endpoint_store is store
        assert "阿里云" in window.login_page.cloud_target_label.text()
...
def test_environment_override_is_session_only_and_preserves_custom_aliyun_route(
    qtbot,
    synthetic_database,
    mock_cloud,
    tmp_path,
    ...
    factory, clients, requests = mock_cloud
    store = EndpointSettingsStore(tmp_path / "custom-settings.json")
    saved = EndpointSettings("cloud", "aliyun", "https://first.example.com/a")
    store.save(saved)
    before = store.path.read_bytes()
