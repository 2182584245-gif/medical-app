...
本版在 1.6.0 的桌面生活智能体基础上增加独立授权的开发者界面：用户业务资料、
AI 与审核知识、功能开关、应用级服务运行设置。只开发 Windows EXE，不修改 Android
或网站。保留离线缓存、阿里云同步、医疗/环境记录、分段时间与账号隔离。
运行方式仍为下载根目录的 **1.7.0 电脑版 ZIP**，完整解压后启动 EXE；
只复制 EXE 会缺少运行库。源代码与构建入口位于本目录。

新增拍下这一餐、活动表达、晨间睡眠核对、本机朗读以及无 Key 规则演示。
AI 生成的记录、偏好和提醒先成为草稿，用户确认后才保存。
默认平台 AI 沿用现有阿里云接口，不需要因升级重新输入 Key；自选个人 DeepSeek
Key 的模式可使用受限多轮工具调用。Windows 主动勾选保存的个人 Key 使用本机
Windows 用户加密，不进入发行 ZIP 或 Git。当前仍只开放本地与阿里云两种模式。

发布范围、权限和延迟生效边界见 [1.7.0 发布说明](docs/release-1.7.0.md)。
GitHub 上传和 EXE 发布不表示阿里云已部署新后端；云端开发者功能需要单独迁移与授权。
公开包不包含开发者账号密码、开发授权、共享 API Key；预置五名虚构人物的历史范围
为 210 天，主界面不附加“示例”标签，但发布清单保留虚构来源声明。
上版协作者合并记录见 [1.6.0 发布说明](docs/release-1.6.0.md)。
下方的 1.0–1.2 说明以及旧部署验收文档为历史记录，其中 Supabase、旧版一键
打包和纯净版下载等描述不代表当前功能。协作者对根 README 使用的 `fawnsee`
是仓库介绍名称；桌面程序名称继续保留“健康生活服务平台”。

## 桌面生活智能体

普通聊天现在可读取授权生活资料、调用工具提出记录/偏好/提醒草稿，再由会员确认保存。
入口包括语音、餐食照片、活动和晨间睡眠；手机与手环保留数据协议，尚未连接设备。
可不填 Key 运行明确标注的离线流程演示，也可切换 DeepSeek V4 Flash。
详见 [架构与运行说明](docs/life-agent-architecture.md) 和 [验收记录](docs/life-agent-validation.md)。
