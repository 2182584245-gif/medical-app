# 管理员完整平台复制（SQLite 6/7；当前云端 platform_0003）

结构兼容说明：原 SQLite 6 源仍以只读方式接受，不直接修改原库；新 SQLite 7 增加医疗分类。
含医疗记录的复制必须先核验目标已经完成 platform_0003，旧 v2 目标只能接收旧五类记录。
这不是仅改版本号的兼容处理，工具核验真实 category CHECK；升级流程见
[医疗增量升级说明](../../server/PLATFORM_V3_MEDICAL_UPGRADE.md)。旧29业务表/32平台表范围不变。
